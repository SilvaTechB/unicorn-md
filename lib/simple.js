...
try {
    ...
    console.error('💡 Check if @whiskeysockets/baileys is installed: npm install @whiskeysockets/baileys')
    process.exit(1)
}

// Extract baileys components
const baileysDefault = baileysImport.default || baileysImport
const {
...
export function serialize() {
    ...
    if (!proto) {
        console.warn('⚠️ proto is not available, skipping serialize()')
        return
    }
    
    if (!proto.WebMessageInfo) {
        console.error('❌ proto.WebMessageInfo is undefined')
        return
    }
